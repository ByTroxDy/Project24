...
public class IngresoController {
    ...
    private void initTableColumns() {
        // Configurar las columnas de la tabla con propiedades de los modelos
//        idIngresoColumn.setCellValueFactory(cellData -> cellData.getValue().idIngresoProperty().asObject());
    ...
    private void agregarIngreso() throws SQLException {
        ...
        TextField tipoFacturaField = new TextField();
        tipoFacturaField.setPromptText("Tipo de Factura");
        ...
        TextField numeroNochesField = new TextField();
        numeroNochesField.setPromptText("Número de Noches");
        numeroNochesField.setTextFormatter(new TextFormatter<>(change ->
                (change.getControlNewText().matches("\\d*")) ? change : null));

        TextField numeroPersonasField = new TextField();
        numeroPersonasField.setPromptText("Número de Personas");
        numeroPersonasField.setTextFormatter(new TextFormatter<>(change ->
                (change.getControlNewText().matches("\\d*")) ? change : null));
        ...
        descuentoField.setTextFormatter(new TextFormatter<>(change ->
        ...
        totalIVAField.setTextFormatter(new TextFormatter<>(change ->
                (change.getControlNewText().matches("-?\\d*\\.?\\d{0,2}")) ? change : null));

        TextField totalFacturaField = new TextField();
        totalFacturaField.setPromptText("Total Factura");
        totalFacturaField.setTextFormatter(new TextFormatter<>(change ->
                (change.getControlNewText().matches("-?\\d*\\.?\\d{0,2}")) ? change : null));
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == buttonTypeOk) {
                return new Ingreso(
                        tipoFacturaField.getText(),
                        fechaEntradaPicker.getValue().toString(),
                        fechaSalidaPicker.getValue().toString(),
                        Integer.parseInt(numeroNochesField.getText()),
                        Integer.parseInt(numeroPersonasField.getText()),
                        idTarifaComboBox.getValue(),
                        Double.parseDouble(descuentoField.getText()),
                        Double.parseDouble(totalIVAField.getText()),
                        Double.parseDouble(totalFacturaField.getText()),
                        observacionesField.getText(),
                        nifClienteComboBox.getValue(),
    ...
    private void modificarIngreso() throws SQLException {
        ...
        TextField tipoFacturaField = new TextField(selectedIngreso.getTipoFactura());
        
        DatePicker fechaEntradaPicker = new DatePicker();
        fechaEntradaPicker.setValue(LocalDate.parse(selectedIngreso.getFechaEntrada()));
        DatePicker fechaSalidaPicker = new DatePicker();
        fechaSalidaPicker.setValue(LocalDate.parse(selectedIngreso.getFechaSalida()));
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == buttonTypeOk) {
                return new Ingreso(
                    ...
                    Double.parseDouble(totalFacturaField.getText()),
                    observacionesField.getText(),
                    selectedIngreso.getIdApartamento(),
                    nifClienteComboBox.getValue(),
                    idIntermediarioComboBox.getValue()
                );
            }
    ...
    private void eliminarIngreso() {
        ...
        alert.setContentText("ID: " + selectedIngreso.getIdIngreso() + "\nTipo de Factura: " + selectedIngreso.getTipoFactura());
